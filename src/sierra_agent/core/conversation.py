...
class CustomerContext:
    ...
    def update_sentiment(self, sentiment: SentimentType) -> None:
        ...
        print(f"😊 [CONTEXT] Sentiment history now has {len(self.sentiment_history)} entries")
...
class Conversation:
    ...
    def add_user_message(self, content: str) -> None:
        ...
        print(f"👤 [CONVERSATION] Adding user message: '{content[:30]}{'...' if len(content) > 30 else ''}'")
        ...
        print(f"👤 [CONVERSATION] User message added. Total messages: {len(self.messages)}")
    
    def add_ai_message(self, content: str) -> None:
        """Add an AI message to the conversation."""
        print(f"🤖 [CONVERSATION] Adding AI message: '{content[:30]}{'...' if len(content) > 30 else ''}'")
        ...
        print(f"🤖 [CONVERSATION] AI message added. Total messages: {len(self.messages)}")
    
    def add_system_message(self, content: str) -> None:
        """Add a system message to the conversation."""
        print(f"⚙️ [CONVERSATION] Adding system message: '{content[:30]}{'...' if len(content) > 30 else ''}'")
        ...
        self.messages.append(message)
        self.last_activity = datetime.now()
        print(f"⚙️ [CONVERSATION] System message added. Total messages: {len(self.messages)}")
    
    def update_customer_sentiment(self, sentiment: SentimentType) -> None:
        ...
        if sentiment == SentimentType.NEGATIVE:
            ...
            print(f"😊 [CONVERSATION] Set urgency to HIGH due to negative sentiment")
            ...
            print(f"😊 [CONVERSATION] Set urgency to NORMAL and satisfaction to 0.8 due to positive sentiment")
    
    def update_quality_score(self, score: float) -> None:
        """Update conversation quality score."""
        print(f"📊 [CONVERSATION] Updating quality score to: {score:.2f}")
        self.quality_score = score
        print(f"📊 [CONVERSATION] Quality score updated successfully")
    
    def _update_conversation_state(self, content: str) -> None:
        """Update conversation state based on message content."""
        print(f"📝 [CONVERSATION] Updating conversation state based on content...")
        ...
        if any(word in content_lower for word in ["order", "tracking", "shipping"]):
            ...
            print(f"📝 [CONVERSATION] Detected topic: order_management")
        elif any(word in content_lower for word in ["product", "gear", "boots", "tent", "hiking"]):
            self.conversation_state.current_topic = "product_inquiry"
            print(f"📝 [CONVERSATION] Detected topic: product_inquiry")
        elif any(word in content_lower for word in ["return", "refund", "complaint", "issue"]):
            self.conversation_state.current_topic = "customer_service"
            print(f"📝 [CONVERSATION] Detected topic: customer_service")
        
        # Urgency detection
        if any(word in content_lower for word in ["urgent", "asap", "emergency", "problem", "broken"]):
            self.conversation_state.urgency_level = "high"
            print(f"📝 [CONVERSATION] Detected high urgency")
    
    def _update_conversation_phase(self) -> None:
        """Update conversation phase based on message count."""
        print(f"🔄 [CONVERSATION] Updating conversation phase...")
    ...
    def get_user_messages(self) -> List[Message]:
        ...
        print(f"👤 [CONVERSATION] Getting user messages...")
    ...
    def get_ai_messages(self) -> List[Message]:
        """Get all AI messages."""
        print(f"🤖 [CONVERSATION] Getting AI messages...")
        ai_messages = [msg for msg in self.messages if msg.message_type == MessageType.AI]
        print(f"🤖 [CONVERSATION] Found {len(ai_messages)} AI messages")
        return ai_messages
    
    def get_system_messages(self) -> List[Message]:
        """Get all system messages."""
        print(f"⚙️ [CONVERSATION] Getting system messages...")
        system_messages = [msg for msg in self.messages if msg.message_type == MessageType.SYSTEM]
    ...
    def get_customer_sentiment_trend(self) -> List[SentimentType]:
    ...
    def get_conversation_patterns(self) -> Dict[str, Any]:
        ...
        print(f"📊 [CONVERSATION] Analyzing conversation patterns...")
    ...
    def clear_conversation(self) -> None:
        ...
        print(f"🧹 [CONVERSATION] Clearing conversation...")
        ...
        print(f"🧹 [CONVERSATION] Conversation cleared and reset")
        logger.info("Conversation cleared and reset")
    
    def export_conversation(self) -> Dict[str, Any]:
        """Export conversation data for analysis."""
        print(f"📤 [CONVERSATION] Exporting conversation data...")
